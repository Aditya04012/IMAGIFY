 ...
 const onClickHanler=()=>{
  ...
  return (
    ...
    >
     ...
     className=' text-center max-w-xl mx-auto mt-5'>Unleash your creativity with Ai. Turn your imagination into visual art in seconds-just type,and watch the magic happen </motion.p>
      ...
      <motion.button
       ...
       onClick={onClickHanler} className='sm:text-lg cursor-pointer text-white bg-black w-auto mt-8 px-12 py-2.5 flex items-center gap-2 rounded-full'>Generate Images
    ...
    <motion.div
     ...
     className='flex gap-3 mt-16 justify-center flex-wrap'>
        {Array(6).fill('').map((item,index)=>(
         <img className='rounded hover:scale-105 transition-all duration-300 cursor-pointer max-sm:w-10'
          src={index%2==0? assets.sample_img_2:assets.sample_img_1} key={index}
           width={70} ></img>
        ))}
      ...
      <motion.p
       ...
       className='mt-2 text-neutral-600'>Generate image from imagify</motion.p>
    </motion.div>
  )
}
